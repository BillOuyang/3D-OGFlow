...
from pointconv_selfocc_util import PointConv, PointConvD, Warping, UpsampleFlow, Occ_weighted_CV
from pointconv_selfocc_util import SceneFlow_Estimator
from pointconv_selfocc_util import index_points_gather as index_points, index_points_group, Conv1d, knn_point
import time
...
class PointConvSceneFlowPWC8192selfglobalPointConv(nn.Module):
    def __init__(self):
        ...
        self.cost0 = Occ_weighted_CV(flow_nei, 32 + 32 + 32 + 32 + 3, [32, 32], [64])
        self.flow0 = SceneFlow_Estimator(32 + 64, 32)
        self.level0_2 = Conv1d(32, 64)

        # l1: 2048
        self.level1 = PointConvD(2048, feat_nei, 64 + 3, 64)                        ## 2048
        self.cost1 = Occ_weighted_CV(flow_nei, 64 + 32 + 64 + 32 + 3, [64, 64], [64])
        self.flow1 = SceneFlow_Estimator(64 + 64, 64)
        self.level1_0 = Conv1d(64, 64)
        self.level1_1 = Conv1d(64, 128)

        # l2: 512
        self.level2 = PointConvD(512, feat_nei, 128 + 3, 128)                       ## 512
        self.cost2 = Occ_weighted_CV(flow_nei, 128 + 64 + 128 + 64 + 3, [128, 128], [64])
        self.flow2 = SceneFlow_Estimator(128 + 64, 128)
        self.level2_0 = Conv1d(128, 128)
        self.level2_1 = Conv1d(128, 256)

        # l3: 256
        self.level3 = PointConvD(256, feat_nei, 256 + 3, 256)                       ## 256
        self.cost3 = Occ_weighted_CV(flow_nei, 256 + 64 + 256 + 64 + 3, [256, 256], [64], bottom_level=True)
        self.flow3 = SceneFlow_Estimator(256, 256, flow_ch=0)
    ...
    def forward(self, xyz1, xyz2, color1, color2):
        ...
        cost3, occ_mask3 = self.cost3(pc1_l3, pc2_l3, c_feat1_l3, c_feat2_l3)  ##[B,c=256,S]
        feat3, flow3 = self.flow3(pc1_l3, feat1_l3, cost3, occ_mask=occ_mask3)
        ...
        cost2, occ_mask2 = self.cost2(pc1_l2, pc2_l2_warp, c_feat1_l2, c_feat2_l2, up_occ_mask2)

        feat3_up = self.upsample(pc1_l2, pc1_l3, feat3)
        new_feat1_l2 = torch.cat([feat1_l2, feat3_up], dim=1)
        feat2, flow2 = self.flow2(pc1_l2, new_feat1_l2, cost2, up_flow2, occ_mask2)
        ...
        cost1, occ_mask1 = self.cost1(pc1_l1, pc2_l1_warp, c_feat1_l1, c_feat2_l1, up_occ_mask1)

        feat2_up = self.upsample(pc1_l1, pc1_l2, feat2)
        new_feat1_l1 = torch.cat([feat1_l1, feat2_up], dim=1)
        feat1, flow1 = self.flow1(pc1_l1, new_feat1_l1, cost1, up_flow1, occ_mask1)
        ...
        cost0, occ_mask0 = self.cost0(pc1_l0, pc2_l0_warp, c_feat1_l0, c_feat2_l0, up_occ_mask0)

        feat1_up = self.upsample(pc1_l0, pc1_l1, feat1)
        new_feat1_l0 = torch.cat([feat1_l0, feat1_up], dim=1)
        _, flow0 = self.flow0(pc1_l0, new_feat1_l0, cost0, up_flow0, occ_mask0)
