...
def eval_one_epoch(model, loader, batchsize):
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        occ_sum += occ_acc
        occ_total_loss += occ_loss * batchsize
...
def main(num_points, batch_size, epochs, use_multi_gpu, pretrain):
    ...
    # F3D dataloader
    train_dataset = SceneflowDataset(npoints=4096, train=True, cache=None)
...
if __name__ == '__main__':
    ...
    # Args
    parser = argparse.ArgumentParser(description='supervised training for 3D-OGFlow.')
